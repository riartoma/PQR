...
AND Fecha_Creacion BETWEEN '2022-01-01' AND '2022-12-31'
GROUP BY mes, estado
LIMIT 0, 25;";
    ...
    <tbody>
      <?php foreach ($datos_meses as $mes => $cantidad) { ?>
        <tr>
          ...
          <td><?php $nombreMes = nombreMes($mes); 
            echo $nombreMes." del año ". $ano; ?></td>
          <td><?php echo (int)$cantidad["Peticion"]; ?></td>
          <td><?php echo (int)$cantidad["Queja"]; ?></td>
          <td><?php echo (int)$cantidad["Reclamo"]; ?></td>
        </tr>
      <?php } ?>
    </tbody>
  </table> 
...
<center><p>&copy; Ricardo Arturo Torres Manrique <?php echo date("Y");?></p
    ...
    pdf.text(200, 50, 'Grafico resumiendo el movimiento');
    pdf.text(200,70,' de las PQR en con el paso de los años!');
   
    pdf.addImage(grafico.toDataURL("image/jpeg"), 'JPEG', margins.left, margins.top+70, margins.width, 0);
    ...
    pdf.fromHTML(
        ...
        margins.top + 50, { // Cambie 220 por la altura deseada para la tabla
        'width': margins.width,
